...
static int uart_stm32_parameters_set(const struct device *dev, const struct uart_config *cfg)
{
	const struct uart_stm32_config *config = dev->config;
	struct uart_stm32_data *data = dev->data;
			       ...
			       int32_t timeout)
{
	...
	if (IS_ENABLED(CONFIG_UART_STM32U5_ERRATA_DMAT_LOWPOWER)) {
		...
		if (config->lin_mode) {
			LL_USART_RequestBreakSending(usart);
		}
		/* Enable TX DMA requests */
		uart_stm32_dma_tx_enable(dev);
	}
...
#define STM32_UART_CHECK_DT_PARITY(index)                                                          \
	...
	BUILD_ASSERT(!(DT_INST_ENUM_IDX(index, parity) != UART_CFG_PARITY_NONE &&                  \
...
#define STM32_UART_CHECK_DT_DATA_BITS(index)                                                       \
	BUILD_ASSERT(!(DT_INST_ENUM_IDX(index, data_bits) == UART_CFG_DATA_BITS_5 ||               \
...
#define STM32_UART_CHECK_DT_STOP_BITS_0_5(index)                                                   \
	BUILD_ASSERT(DT_INST_ENUM_IDX(index, stop_bits) != UART_CFG_STOP_BITS_0_5,                 \
		     "Node " DT_NODE_PATH(                                                         \
...
#define STM32_UART_CHECK_DT_STOP_BITS_1_5(index)                                                   \
	BUILD_ASSERT(DT_INST_ENUM_IDX(index, stop_bits) != UART_CFG_STOP_BITS_1_5,                 \
		     "Node " DT_NODE_PATH(                                                         \
			     DT_DRV_INST(index)) " has unsupported stop bits configuration");
...
#define STM32_UART_INIT(index)                                                                     \
	...
	static const struct uart_stm32_config uart_stm32_cfg_##index = {                           \
		...
		.lin_mode = DT_INST_PROP(index, lin_mode),                                         \

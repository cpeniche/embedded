...
void OSBeeWiFi::options_setup() {
  if(!LittleFS.exists(config_fname)) { // if config file does not exist
...
void OSBeeWiFi::options_reset() {
  ...
  if(!LittleFS.remove(config_fname)) {
...
void OSBeeWiFi::options_load() {
  File file = LittleFS.open(config_fname, "r");
...
void OSBeeWiFi::options_save() {
  File file = LittleFS.open(config_fname, "w");
...
void OSBeeWiFi::write_log(const LogStruct& data) {
  ...
  if(!LittleFS.exists(log_fname)) {  // create log file
    file = LittleFS.open(log_fname, "w");
    if(!file) {
      DEBUG_PRINTLN(F("failed"));
      return;
    }
    ...
    file = LittleFS.open(log_fname, "r+");
    if(!file) {
      DEBUG_PRINTLN(F("failed"));
      return;
    }
...
bool OSBeeWiFi::read_log_start() {
  ...
  log_file = LittleFS.open(log_fname, "r");
